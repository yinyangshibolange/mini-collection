...
const { deleteFolderRecursive, deepFold, findAndMkdir, copyFold} = require("./fs-utils")
...
// 字体压缩，实际是生成woff2等字体文件
// function mf(fold, targetFold) {
...
//     })
// }
// async function miniFont(fold, targetFold) {
// await  mf(fold, targetFold)
// }
...
async function deal_sf (config, sf) {
    ...
    if(file) {
        const targetPath = path.resolve(process.cwd(), config.build.targetFold, rename || file)
        const srcPath = path.resolve(process.cwd(), config.build.srcFold, file)
        await fs_promises.copyFile(srcPath, targetPath)
    } else {
        if(use) { // 使用处理器
            const targetPath = path.resolve(process.cwd(), config.build.targetFold, tfold || fold)
            if(use === 'postcss') {
                logger.primary('css处理中.....')
                try {
                    await minicss(path.resolve(process.cwd(), config.build.srcFold, fold), targetPath, ext)
                } catch(err) {
                    console.error(err)
                }
                ...
                try {
                    await miniImage(`${config.build.srcFold}/${fold}`, targetPath, ext)
                } catch(err) {
                    console.error(err)
                }
                ...
                try {
                    await uglifyJsDeal(path.resolve(process.cwd(), config.build.srcFold, fold), targetPath, ext)
                } catch(err) {
                    console.error(err)
                }
            }
            // else if(use === 'fontmin') {
            //     try {
            //         await miniFont(path.resolve(process.cwd(), config.build.srcFold, fold), targetPath)
            //     } catch(err) {
            //         console.error(err)
            //     }
            //
...
async function dealStaticThings (config) {
    const static_folds = config.build.loaders
...
function watchAndCompile(config, ) {
    ...
    const fileFold = path.resolve(process.cwd(), config.build.srcFold)
    // 监听文件变化
    watcher = watchFiles(fileFold, {
        recursive: true,
        filter(f, skip) {
            for(let ld of config.build.loaders) {
